...
def preprocess_function(examples):
    ...
    model_inputs = tokenizer(inputs, max_length=128, truncation=True, padding="max_length", return_tensors="pt").to(device)
    labels = tokenizer(targets, max_length=128, truncation=True, padding="max_length", return_tensors="pt").to(device)

    bos_token_id = tokenizer.bos_token_id
    decoder_input_ids = torch.full((labels.input_ids.shape[0], 128), bos_token_id if bos_token_id is not None else tokenizer.pad_token_id).to(device)
